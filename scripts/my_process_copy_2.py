...
static_obstacle_polygon = shapely.ops.unary_union(land_polygons + bridge_polygons)
...
boundary_polygons = shapely.geometry.MultiPolygon(boundary_polygons)

image_utm_ratio = 2.0
half_image_dim = 640

offset_rpy = scipy.spatial.transform.Rotation.from_euler('xyz', [0, 180, -90], degrees=True).as_matrix()
...
def process(radar_data, jetyak_config, time_stamp):
    ...
    radar_data['ypr'].append(radar_data['ypr'][0])

    radar_data['angle'] = np.pi * 2 * np.array(radar_data['angle']) / 4096.
    radar_data['utm'] = np.array(radar_data['utm'])
    radar_data['ypr'] = np.array(radar_data['ypr'])
    ...
    for radar_contour in radar_contours:
        ...
        for ypr, angle, distance, utm_offset in zip(radar_data['ypr'], radar_data['angle'], distances, utm_offsets):

            print(np.degrees(angle))
            ypr_matrix = scipy.spatial.transform.Rotation.from_euler('zyx', ypr).as_matrix()
            angle_matrix = scipy.spatial.transform.Rotation.from_euler('z', angle).as_matrix()
            radar_contour.append(angle_matrix.dot(np.array([distance, 0, 0]))[:2] + utm_offset)
            # print(offset_rpy.dot(ypr_matrix).dot(angle_matrix).dot(np.array([distance, 0, 0])))

        radar_contour = np.array(radar_contour)
        ...
        if len(radar_contour) >= 3:
            ...
            fill_polygon_alpha(radar_polygon, (220, 220, 220), 0.6)
    ...
    for radar_polygon in radar_polygons:
        ...
        if radar_polygon.area > 10:
            ...
            # if radar_polygon.intersects(static_obstacle_polygon):

            #     fill_polygon_alpha(radar_polygon,
            #                        (220, 220, 220), 0.4)
            ...
            #                 target['x'][1] - jetyak_utm[1], target['x'][0] - jetyak_utm[0]))) < np.pi / 2
            ...
            # obstacle_polygons = shapely.ops.unary_union([obstacle_polygons, boundary_polygons])
